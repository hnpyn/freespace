...
void FreeSpace::get_free_space(
    const std_msgs::Header header, const PointICloudPtr &point_cloud,
    std::vector<int> &list, lidar::RoiPoint roi_point,
    PointICloudPtr out_point_cloud, PointICloudPtr out_point_cloud_xy,
    visualization_msgs::MarkerArray &free_space_marker) {
  std::clock_t startTime, endTime;
  int polar_grid_nums_ = std::ceil((config_.angle_end - config_.angle_begin) /
                                   config_.angle_resolution);

  // 建立极坐标栅格
  startTime = clock();
  int gridRows = std::ceil((config_.distance_max - config_.distance_min) /
                           config_.distance_resolution);
  ...
  std::vector<std::vector<grid_feature>> polar_grid_points(
      gridRows,
      std::vector<grid_feature>(gridCols)); // 最大高度 最小高度 最小距离 点个数
  ...
  // 判定栅格属性
  startTime = clock();
  std::vector<std::vector<int>> objects_grid(gridRows,
                                             std::vector<int>(gridCols, 0));
  ...
  get_closest_distance_2(polar_grid_points, objects_grid, gridRows, gridCols,
                         point_list);
  endTime = clock();
...
void FreeSpace::transform_formats(std::vector<PointI> &out_point_list_xy,
                                  std::vector<int> &list) {
...
void FreeSpace::get_point_list_xy(std::vector<double> &out_point_list,
  ...
  for (int i = 0; i < listSize; i++) {
    ...
    if (i < halfListSize)
      theta =
          get_theta(i) + config_.t_angle_begin + config_.angle_resolution / 2;
    else
      theta = get_theta(i) + config_.t_angle_begin +
              config_.angle_resolution / 2 + config_.t_angle_free * 2;
...
void FreeSpace::shrink_angle(std::vector<double> &point_list,
                             std::vector<double> &out_point_list) {
  double remain_angle =
      90.0 - config_.t_angle_free - (config_.t_angle_begin - 90.0);
...
void FreeSpace::correct_points_angle(std::vector<double> &point_list) {
  double former_angle = std::ceil(
      (config_.t_angle_begin - config_.angle_begin) / config_.angle_resolution);
  double back_angle =
      std::ceil((config_.t_angle_end) / config_.angle_resolution);
...
bool FreeSpace::check_corrected_points(int i,
                                       std::vector<int> points_has_corrected) {
...
void FreeSpace::correct_freespace(lidar::RoiPoint roi_point,
                                  ...
                                  PointICloudPtr &temp_point_cloud) {
  ...
  for (int i = 0; i < roiSize; i++) {
    ...
    if (twoPoints.size() > 1) {
      ...
      for (int j = start_num; j < listSize; j++) {
        ...
        // std::cout << "firstPoint: " << firstPoint.ag << " secondPoint: " <<
        // secondPoint.ag << std::endl;
      ...
      for (int k = 1; k < n; k++) {
        ...
        double freeSpaceDistance =
            calculate_dis(out_point_cloud->points.at(m + k - 1));
        // std::cout << "debug: ..." << std::endl;
        if (correctedDis < freeSpaceDistance) {
          distance = correctedDis;
        } else {
          distance = freeSpaceDistance;
        }
        get_point_position(theta, distance, outPoint);
  ...
  // std::cout << "debug: "
  //                    << "\n" << "points for draw: " <<
  //                    temp_point_cloud->points.size()
  //                    << "\n" << "total correct nums: " << t_correctNum <<
  //                    std::endl;
}

#include <pcl/filters/voxel_grid.h>
void FreeSpace::draw_lines(const std_msgs::Header header,
                           PointICloudPtr out_point_cloud,
                           visualization_msgs::MarkerArray &free_space_marker) {
...
void FreeSpace::filter_points_cloud(const PointICloudPtr point_cloud,
                                    PointICloudPtr filter_point_cloud) {
...
// 转换到点云坐标
void FreeSpace::draw_points(const std::vector<double> &point_list,
                            PointICloudPtr &out_point_cloud) {
  ...
  for (int i = 0; i < listSize; i++) {
    ...
    //   {
    //     distance = (1 + distance_ / (10 * (distance - distance_))) *
    //     distance_ + 1;
    //   }
    //   else
    //   {
    ...
    //   {
    //     distance = (1 + distance_ / (20 * (distance - distance_))) *
    //     distance_ + 1;
    //   }
    //   else
    //   {
    ...
    // writeFile.open("/home/wang/driveworks/freespace_lidar/src/handle/txt/distance.txt",
    // ios_base::app); writeFile << distance; writeFile << "\n";
    // writeFile.close();
  ...
  //   std::ofstream writeFile;
  //   writeFile.open("/home/wang/driveworks/freespace_lidar/src/handle/txt/points.txt",
  //   ios_base::app); writeFile << out_point_cloud->points.back(); writeFile <<
  //   "\n"; writeFile.close();
}

void FreeSpace::get_point_position(const double theta, const double distance,
                                   PointI &out_point) {
...
void FreeSpace::get_closest_distance_2(
    std::vector<std::vector<grid_feature>> &polar_grid_points,
    const std::vector<std::vector<int>> &objects_grid, const int nRows,
    const int nCols, std::vector<double> &point_list) {
  for (int i = 0; i < nCols; i++) {
    for (int j = 0; j < nRows; j++) {
      if (objects_grid[j][i] == 1) {
        ...
        // 不使用curb
        double closestDis = polar_grid_points[j][i].closestDis;
        point_list.at(i) = (j + 0.5) * config_.distance_resolution +
                           config_.distance_min; // closestDis;
        break;
      }
    }
  }
...
void FreeSpace::get_closest_distance(
    std::vector<std::vector<grid_feature>> &polar_grid_points,
    const std::vector<std::vector<int>> &objects_grid, const int nRows,
    const int nCols, std::vector<double> &point_list) {
  double front_angle =
      90 - atan(config_.distance_max / config_.t_side_distance) * 180 / M_PI;
  for (int i = 0; i < nCols; i++) {
    ...
    if ((theta > 180 - front_angle && theta < 180 + front_angle) ||
        (theta < front_angle) || (theta > config_.angle_end - front_angle)) {
      for (int j = 0; j < nRows; j++) {
        if (objects_grid[j][i] == 1) {
          double closestDis = polar_grid_points[j][i].closestDis;
          point_list.at(i) = closestDis;
...
void FreeSpace::check_grid(
    const std::vector<std::vector<grid_feature>> &polar_grid_points,
    const int rows, const int cols,
    std::vector<std::vector<int>> &objects_grid) {
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      double pointsHeight =
          polar_grid_points[i][j].maxHeight - polar_grid_points[i][j].minHeight;
...
bool FreeSpace::construct_polar_grid(
    const PointICloudPtr &point_cloud,
    std::vector<std::vector<grid_feature>> &polar_grid_points) {
  ...
  for (int i = 0; i < pointsSize; i++) {
    ...
    if (pointTheta <= config_.angle_begin || pointTheta >= config_.angle_end)
      continue;
    int nCols = std::floor((pointTheta - config_.angle_begin) /
                           config_.angle_resolution);
    // 距离值
    double pointDistance = get_points_distance(point_cloud->points[i]);
    int nRows = std::floor((pointDistance - config_.distance_min) /
                           config_.distance_resolution);

    if (pointDistance >= config_.distance_max ||
        pointDistance < config_.distance_min) {
      continue;
    }
    //    std::cout<<"dist: " << pointDistance <<std::endl;
    //    std::cout<<"nRows: " << nRows << ", nCols" << nCols <<std::endl;
    // ROS_INFO("x %f, y %f", point_cloud->points[i].x ,
    // point_cloud->points[i].y);

    // ROS_INFO("rows : %d, cols : %d  dis %f, theta : %f", nRows, nCols,
    // pointDistance, pointTheta);
    ...
    if (pointZ < polar_grid_points[nRows][nCols].minHeight) {
      // ROS_INFO("height : %f, z %f, row: %d, col: %d",
      // polar_grid_points[nRows][nCols].minHeight, pointZ,nRows,nCols);
...
void FreeSpace::onPointCloud(
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr cloudPtr(
      new pcl::PointCloud<pcl::PointXYZI>); // 点云指针
  pcl::PointCloud<pcl::PointXYZI>::Ptr filterPtr1(
      new pcl::PointCloud<pcl::PointXYZI>); // 滤波后的指针
  pcl::PointCloud<pcl::PointXYZI>::Ptr filterPtr2(
      new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr filterPtr3(
      new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr filterPtr4(
      new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr filterPtr5(
      new pcl::PointCloud<pcl::PointXYZI>);
  // 转换成pointT
  ...
  outrem.setMinNeighborsInRadius(5); // 设置查询点的邻域点集数小于2的删除
                                     // apply filter
  outrem.filter(*filterPtr5); // 执行条件滤波   在半径为0.8
  ...
  config_ = fsconfig;
  get_free_space(header, filterPtr5, point_list, roi_point, free_space_cloud,
                 out_point_cloud_xy, free_space_marker);
