...
//#include <lidar_perception/output_object.h>
//#include <lidar_perception/output_object_list.h>
...
namespace common {
                                     ...
                                     const Eigen::Matrix4d& trans = Eigen::Matrix4d::Zero())
    {
        ...
        if ((trans.array() != 0.0).any()) {
            common::transform::transformPointCloud<lidar::PointI>(trans, cloud);
        }
                                      ...
                                      const Eigen::Matrix4d& trans = Eigen::Matrix4d::Zero())
    {
        ...
        if (objects_array.size() <= 0) {
            ...
            ROS_INFO("Publishing %u tracked objects markers.", objects_array.size());
        }
        ...
        for (size_t obj = 0u; obj < objects_array.size(); ++obj) {
            ...
            box.header = dir_arrow.header = text.header = header;
            box.ns = dir_arrow.ns = text.ns = "objects";
            box.id = obj; dir_arrow.id = obj + objects_array.size();
            text.id = obj + objects_array.size() * 2;
            ...
            if (objects_array[obj]->type == 3) { //pedestrian
                color.a = 1;
                color.r = 1;
                color.g = 1;
                color.b = 0;
            }
            else if(objects_array[obj]->type == 4) { //bicycle
                color.a = 1;
                color.r = 0;
                color.g = 0;
                color.b = 1;
            }
            else if (objects_array[obj]->type == 5) { //car
                color.a = 1;
                color.r = 0;
                color.g = 1;
                color.b = 0;
            }
            else if (objects_array[obj]->type == 7) { //car
                color.a = 1;
                color.r = 1;
                color.g = 0;
                color.b = 0;
            ...
            pose.position.x = objects_array[obj]->ground_center(0, 0);
            pose.position.y = objects_array[obj]->ground_center(1, 0);
            pose.position.z = objects_array[obj]->ground_center(2, 0);
            float velocity = sqrt(objects_array[obj]->velocity(0) * objects_array[obj]->velocity(0) +
                                  objects_array[obj]->velocity(1) * objects_array[obj]->velocity(1));
            std::ostringstream str;
            str << "track_id: " << objects_array[obj]->track_id << std::endl <<
                   "track_time: " << objects_array[obj]->tracking_time << std::endl <<
                   "position: " << objects_array[obj]->ground_center(0) << ", " <<
                                   objects_array[obj]->ground_center(1) << ", " <<
                                   objects_array[obj]->ground_center(2) << ", " << std::endl <<
                 "velocity: " << objects_array[obj]->velocity(0) << ", " <<
                                 objects_array[obj]->velocity(1) << ", " <<
                                 objects_array[obj]->velocity(2) << ", " << std::endl;
            text.pose = pose;
            text.text = str.str();
            text.color.a = 1;
            text.color.r = 1.0;
    ...
    static bool compareDist(const Object &a, const Object &b)
    {
      double x, y;
    ...
    static bool compareDistPtr(const ObjectPtr &a, const ObjectPtr &b)
    {
      double x, y;
      double dist_a, dist_b;
      x = a->anchor_point(0);
    ...
    /**
     * @brief publish Objects msg
     * @param publisher
     * @param header
     * @param objects
     */
    /*
    static void publishObjectsMsgs(const ros::Publisher& publisher,
                                      const std_msgs::Header& header,
                                      const std::vector<ObjectPtr>& objects_array)
    {
      std::vector<Object> candidate_object;
      lidar_perception::output_object_list output;

      for (int i = 0; i < objects_array.size(); ++i) {
        double x = objects_array[i]->anchor_point(0);
        double y = objects_array[i]->anchor_point(1);
        double theta = atan2(y, x);
        if (fabs(theta) <= M_PI/3) {
          candidate_object.push_back(*objects_array[i]);
        }
      }

      output.header = header;
      int object_num = candidate_object.size();
      if (object_num > 5) {
        std::sort(candidate_object.begin(), candidate_object.end(), compareDist);
        output.object_num = 5;
        for (int i = 0; i < 5; ++i) {
          output.objects[i].id = candidate_object[i].track_id;
          output.objects[i].life_time = candidate_object[i].tracking_time;
          output.objects[i].confidence = 1.0;
          output.objects[i].x = candidate_object[i].nearest_center[0];
          output.objects[i].y = candidate_object[i].nearest_center[1];
          output.objects[i].z = 0.0;
          output.objects[i].v_x = candidate_object[i].velocity(0);
          output.objects[i].v_y = candidate_object[i].velocity(1);
          output.objects[i].v_z = 0.0;
          output.objects[i].length = candidate_object[i].length;
          output.objects[i].width = candidate_object[i].width;
          output.objects[i].height = candidate_object[i].height;
        }
      }
      else {
        output.object_num = object_num;
        for (int i = 0; i < object_num; ++i) {
          output.objects[i].id = candidate_object[i].id;
          output.objects[i].life_time = candidate_object[i].tracking_time;
          output.objects[i].x = candidate_object[i].nearest_center[0];
          output.objects[i].y = candidate_object[i].nearest_center[1];
          output.objects[i].z = 0.0;
