...
public:
  FreeSpace();
  void get_free_space(const std_msgs::Header header,
                      const PointICloudPtr &point_cloud, std::vector<int> &list,
                      lidar::RoiPoint roi_point, PointICloudPtr out_point_cloud,
  ...
  void onPointCloud(const sensor_msgs::PointCloud2ConstPtr &point_cloud_msg,
                    ...
                    const config fsconfig, lidar::RoiPoint roi_point);

private:
  bool construct_polar_grid(
  ...
  void
  check_grid(const std::vector<std::vector<grid_feature>> &polar_grid_points,
  ...
  void get_closest_distance_2(
      std::vector<std::vector<grid_feature>> &polar_grid_points,
      const std::vector<std::vector<int>> &objects_grid, const int nRows,
      const int nCols, std::vector<double> &point_list);
  void get_point_position(const double theta, const double distance,
                          PointI &out_point_cloud);
  void draw_points(const std::vector<double> &point_list,
                   PointICloudPtr &out_point_cloud);
  void filter_points_cloud(const PointICloudPtr point_cloud,
                           PointICloudPtr filter_point_cloud);
  void draw_lines(const std_msgs::Header header, PointICloudPtr out_point_cloud,
                  visualization_msgs::MarkerArray &free_space_marker);
  ...
  void shrink_angle(std::vector<double> &point_list,
  ...
  void get_point_list_xy(std::vector<double> &out_point_list,
                         ...
                         PointICloudPtr out_point_cloud_xy);
  void transform_formats(std::vector<PointI> &out_point_list_xy,
                         std::vector<int> &list);
