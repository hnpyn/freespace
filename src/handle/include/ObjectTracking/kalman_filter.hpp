...
class KalmanFilter : public baseFilter {
 public:
  ...
  // @return nothing
  void UpdateWithObject(const std::shared_ptr<objects_tracked>& new_object,
 ...
 protected:
  ...
  Eigen::VectorXf ComputeMeasuredVelocity(
      ...
      const std::shared_ptr<objects_tracked>& old_object, const double time_diff);
  ...
  // @param[IN] old_object: old object for last updating
  // @return nothing
  void ComputeUpdateQuality(const std::shared_ptr<objects_tracked>& new_object,
                            const std::shared_ptr<objects_tracked>& old_object);
