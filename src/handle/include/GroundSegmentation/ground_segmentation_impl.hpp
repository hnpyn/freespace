#ifndef GROUNDSEGMENTATION_IMPL_HPP
#define GROUNDSEGMENTATION_IMPL_HPP

#include "ground_segmentation.hpp"

namespace lidar {

static double wrapTo360(double euler_angle) {
    if (euler_angle > 0) {
...
static bool compareSignalPoints(const SignalPoint &a, const SignalPoint &b) {
...
void GroundSegmentation<PointT>::sectorINSAC(int sector_index) {
    ...
    for (int i = 0; i < this->params.num_bins_l; i++) {
        if (this->polar_bin_grid.ang_cells[sector_index].lin_cell[i].bin_indices.size() > 0 &&
            this->polar_bin_grid.ang_cells[sector_index].range_height_signal[i].y <
            this->params.max_seed_height) {
          ...
          for (int j = 0; j < this->polar_bin_grid.ang_cells[sector_index].lin_cell[i].bin_indices.size(); ++j) {
            int idx = this->polar_bin_grid.ang_cells[sector_index].lin_cell[i].bin_indices[j];
            this->obs_indices.push_back(idx);
          }
        }
    }
...
//            fabs(sig_points[curr_idx].height) < this->params.max_seed_height) {
        if (sig_points[curr_idx].range < 10.0 && fabs(sig_points[curr_idx].height) < 0.1
            || sig_points[curr_idx].range >= 5.0 && sig_points[curr_idx].range <= this->params.max_seed_range
            && fabs(sig_points[curr_idx].height < 0.5)
            ) {
            // close enough to
            // robot and height
            // makese sense in
    ...
    if (sufficient_model) {
      ...
      const auto &nonModelSegment = this->polar_bin_grid.ang_cells[sector_index];
      for (int ii = 0; ii < nonModelSegment.lin_cell.size(); ++ii) {
        for (int jj = 0; jj < nonModelSegment.lin_cell[ii].bin_indices.size(); ++jj) {
          int curIdx = nonModelSegment.lin_cell[ii].bin_indices[jj];
          const auto &cur_point = (*this->input_)[curIdx];
          if (cur_point.z > 0.3) {
            this->obs_indices.push_back(curIdx);
          }
          else {
            this->ground_indices.push_back(curIdx);
          }
        }
        //std::cout << sector_index <<": WARNING:Insufficient Model for angular slice  " << current_model.size() << std::endl;
      }
    }
}
...
#endif  // GROUNDSEGMENTATION_IMPL_HPP
